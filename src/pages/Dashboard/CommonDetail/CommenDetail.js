...
import CopllapseDetail from '@/components/myComponents/CopllapseDetail';
...
const text = (
  <p style={{ paddingLeft: 24 }}>
    ...
    found:a welcome guest in many households across the world.
  </p>
);
...
class Articles extends PureComponent {
  ...
  columns = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <Fragment>
          <a onClick={() => this.handleUpdateModalVisible(true, record)}>置顶</a>
  ...
  renderHeader(key) {
    ...
    return (
      <div>
        {this.stringText}
        {type === 'comments' ? '评论' : '作品'}
        {key === '展开' ? <Icon type="down" /> : <Icon type="up" />}
      </div>
    );
  }
