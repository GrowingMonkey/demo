...
import mystyles from './AddTeam.less';
...
@connect(({ loading }) => ({
  submitting: loading.effects['form/submitRegularFormTwo'],
}))
@Form.create()
class BasicForms extends PureComponent {
  ...
  handleSubmit = e => {
    ...
    fileListTwo.map((item, index) => (strUrltwo += `${item.imageUrl  },`));
    e.preventDefault();
    form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        dispatch({
          type: 'form/submitRegularFormTwo',
          payload: {
            name: values.name,
            phone: values.phone,
            mail: values.mail,
            contractPath: strUrl,
            licensePath: strUrltwo,
          },
        });
      }
    });
  ...
  render() {
    ...
    const fileListTwo = this.state ? this.state.fileListTwo : '';
    return (
      <PageHeaderWrapper content={content}>
        <Card bordered={false}>
          <Form onSubmit={this.handleSubmit} hideRequiredMark style={{ marginTop: 8 }}>
            <FormItem {...formItemLayout} label={<FormattedMessage id="公司/团队名" />}>
            ...
            <FormItem {...formItemLayout} label={<FormattedMessage id="联系电话" />}>
            ...
            <FormItem {...formItemLayout} label={<FormattedMessage id="营业执照" />}>
            ...
            <FormItem wrapperCol={{ span: 12, offset: 7 }}>
              <div>
                {fileListTwo.map(function(item, index) {
                  return (
                    <img
                      src={item ? item.imageUrl : ''}
                      style={{ width: 100, height: 100, marginRight: 20 }}
