...
@connect(({ bannerlist, loading }) => ({
  bannerlist,
  loading: loading.models.bannerlist,
}))
@Form.create()
class TableList extends PureComponent {
        ...
        if (record.stat == 0) {
          return (
            <Fragment>
              ...
              <a onClick={() => this.handleCancleBanner('down', record)}>下架</a>
            </Fragment>
          );
        } 
          return (
            <Fragment>
              <a onClick={() => this.handleUpdateModalVisible(true, record)}>查看</a>
              <Divider type="vertical" />
              <a onClick={() => this.handleCancleBanner('up', record)}>上架</a>
            </Fragment>
          );
  ...
  handleUpdateModalVisible = (flag, record) => {
    // router.push(`/list/bannerdetail?bid=${record.id}`)
    router.push({ pathname: `/list/bannerdetail`, params: record });
  };

  handleCancleBanner = (flag, record) => {
    console.log(1);
    const { dispatch } = this.props;
    dispatch({
      type: 'bannerlist/cancle',
      payload: {
        id: record.id,
        oprType: flag,
      },
